...
### 🔬 Tracing the recursive calls, step by step

The table above shows *what* happens. This is *why* it happens — the actual call-by-call trace of `hanoi(3, 'A', 'C', 'B')`, unwinding one function call at a time.

> [!TIP]
> Every call that starts must eventually finish, and calls only finish in the reverse order they started — last one in, first one out. That's the **call stack** at work, and it's the engine behind every recursive function.

<details>
<summary>📜 Click to expand the full step-by-step trace</summary>

1. 🔵 `hanoi(3, A, C, B)` starts. `n` isn't 0, so it calls `hanoi(2, A, B, C)` first.
2. 🔵 `hanoi(2, A, B, C)` starts. It calls `hanoi(1, A, C, B)` first.
3. 🔵 `hanoi(1, A, C, B)` starts. It calls `hanoi(0, ...)` — that just returns, does nothing.
4. 🖨️ Back in **step 3**: prints `"Move disk 1 from A to C"`. Then calls `hanoi(0, ...)` again — nothing happens.
5. ↩️ **Step 3** is now finished. Control returns to **step 2**.
6. 🖨️ Back in **step 2**: prints `"Move disk 2 from A to B"`.
7. 🔵 **Step 2** now calls `hanoi(1, C, B, A)`. It calls `hanoi(0)` — nothing — then prints `"Move disk 1 from C to B"` — then `hanoi(0)` again — nothing.
8. ↩️ **Step 2** is now finished. Control returns to **step 1**.
9. 🖨️ Back in **step 1** (the original call): prints `"Move disk 3 from A to C"`.
10. 🔵 **Step 1** now calls `hanoi(2, B, C, A)`. This repeats the same pattern as steps 2–7, but on poles B and C:
    1. calls `hanoi(1, B, A, C)` → prints `"Move disk 1 from B to A"`
    2. prints `"Move disk 2 from B to C"`
    3. calls `hanoi(1, A, C, B)` → prints `"Move disk 1 from A to C"`
11. ✅ Everything is done. No more calls left.

</details>

**Legend:** 🔵 a call starts &nbsp;•&nbsp; 🖨️ a move gets printed &nbsp;•&nbsp; ↩️ a call finishes and hands control back &nbsp;•&nbsp; ✅ the whole recursion is complete

📤 **And here's the final printed output, top to bottom:**
